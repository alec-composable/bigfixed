...
#[test]
fn fix_position() {
    let mut zero_bit = BigFixedVec::<D> {
        head: 0,
        body: vec![],
        position: Bit(0)
    };
    zero_bit.fix_position().unwrap();
    let zero_pos = BigFixedVec::<D> {
        head: 0,
        body: vec![],
        position: Position(0)
    };
    assert_eq!(zero_bit, zero_pos, "zero [0] vs zero (0)");
    let mut zero_bit2 = BigFixedVec::<D> {
        head: 0,
        body: vec![],
        position: Bit(1)
    };
...
fn format() {
    ...
    assert_eq!(BigFixedVec::<D>::construct(D::ZERO, vec![0; 10], Bit(-140)).unwrap(), real_zero, "zero zeroes");
    ...
    assert_eq!(BigFixedVec::<D>::construct(D::ALLONES, vec![0, 0, 5, D::ALLONES], Bit(D::DIGITBITS as isize * -1)).unwrap(), real_t1, "negative with body both sides");
    println!("all good");
}